...
export async function savePatient(clinicId: number, input: PatientInput) { const client = requireClient(); const phone = input.phone.trim().replace(/[^0-9]/g, '') || input.phone.trim(); const payload = { name: input.name.trim(), phone, email: input.email.trim() || null, address: input.address.trim() || null, birth_date: input.birthDate || null }; const query = input.id ? client.from('patients').update(payload).eq('id', input.id) : client.from('patients').insert({ clinic_id: clinicId, ...payload }); const { error } = await query; if (error) throw error }
